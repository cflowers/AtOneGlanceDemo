...
public class CBSceneWineGlassE : AbstractCBScene
{
    private Done d;
    ...
    {
        c.setCanvas("canvas");
        createWineGlassEButton();
        createBackButton();
        d.done = true;
        base.setButtons(c.getButtons());
    }

    private void createWineGlassEButton()
    {
        dicAnchor["anchorMin"] = new Vector2(0.5f, 0.5f);
        dicAnchor["anchorMax"] = new Vector2(0.5f, 0.5f);
        dicAnchor["buttonPos"] = new Vector2(-239, 4);
        if (!(Inspection.getWineGlassEInsp()))
        c.createButtons("buttonWineGlassE", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate { lis_Inspection(); }),
          false, true);
