...
public class CBScenePick : AbstractCBScene
{
    private Done d;
    ...
    private void createPickButton()
    {
        ...
        if (!(Inspection.getPick()))
        c.createButtons("buttonPick", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate {lis_pickInspection(); }),
          false, true);
        else if (Inspection.getPick())
            c.createButtons("buttonPick", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate { lis_pickInspection(); }),
              false, false);
    }
    ...
    public void lis_pickInspection()
    {
        destroyButtons();
