...
public class CBSceneAwardZoom : AbstractCBScene
{
    private Done d;
    ...
    public void getSceneButtons()
    {
        ...
        c.setCanvas("canvas");
        createAwardInspectButton();
        createBackButton();
        d.done = true;
        base.setButtons(c.getButtons());
    }

    private void createAwardInspectButton()
    {
        dicAnchor["anchorMin"] = new Vector2(0.5f, 0.5f);
        dicAnchor["anchorMax"] = new Vector2(0.5f, 0.5f);
        dicAnchor["buttonPos"] = new Vector2(36, 28);
        if (!(Inspection.getAward()))
        c.createButtons("buttonAwardInspect", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate {lis_inspection(); }),
          false, true);
        else if (Inspection.getAward())
        c.createButtons("buttonAwardInspect", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate { lis_inspection(); }),
        false, false);
    }
