...
public class CBSceneClk : AbstractCBScene
{
    private Done d;
    ...
    private void createClockButton()
    {
        ...
        if (!(Inspection.getClock()))
        c.createButtons("buttonClock", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate {lis_inspection(); }),
          false, true);
        
        else if (Inspection.getClock())
            c.createButtons("buttonClock", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate { lis_inspection(); }),
         false, false);
    }
