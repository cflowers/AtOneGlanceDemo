...
public class CBSceneFareed : AbstractCBScene
{
    private Done d;
    ...
    {
        destroyButtons();
        GameObject img = GameObject.FindGameObjectWithTag("fareedMess1Zm");
        GameObject bg = GameObject.FindGameObjectWithTag("canvas");
        img.GetComponent<RawImage>().enabled = true;
        item = new PurplePhoneItem1();
       // this.item.beginText();
        this.item.loadImage();
         JsonBuffer jsonBuffer = new JsonBuffer();
        jsonBuffer.setToggleText("Fareed1");  
        bg.GetComponent<Background>().purplePhone_fareed_zm();
        d.done = false;
        bg.GetComponent<DisplayText>().item = item;
        bg.GetComponent<DisplayText>().popUpPhoneFareed1();
        Inspection.setFareedMess1(true);
    }
    ...
    public void lis_message()
    {
        ...
        GameObject bg = GameObject.FindGameObjectWithTag("canvas");
        img.GetComponent<RawImage>().enabled = true;
        bg.GetComponent<Background>().purplePhone_fareed2();
        d.done = false;
    }
