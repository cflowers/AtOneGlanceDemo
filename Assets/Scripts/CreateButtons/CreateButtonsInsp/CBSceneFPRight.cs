...
public class CBSceneFPRight : AbstractCBScene
{
    private Done d;
    ...
    private void createLogButton()
    {
        ...
        if (!(Inspection.getLogHolder()))
        c.createButtons("buttonLogHolder", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate {lis_logInspection(); }),
          false, true);
        else if (Inspection.getLogHolder())
         c.createButtons("buttonLogHolder", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate { lis_logInspection(); }),
          false, false);
    }
    ...
    private void createBinderButton()
    {
        ...
        dicAnchor["buttonPos"] = new Vector2(-182, 74);
        c.createButtons("buttonBinder", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate { lis_binder(); }),
          false, true);
    }
