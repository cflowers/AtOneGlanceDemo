...
public class CBSceneJarLook : AbstractCBScene
{
    private Done d;
    ...
    private void createJarLookButton()
    {
        ...
        if (!(Inspection.getJar()))
        c.createButtons("buttonJarLook", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate {lis_inspection(); }),
          false, true);

        else if (Inspection.getJar())
            c.createButtons("buttonJarLook", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate { lis_inspection(); }),
              false, false);
    }
