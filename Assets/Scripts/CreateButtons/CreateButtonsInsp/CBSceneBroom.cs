...
public class CBSceneBroom : AbstractCBScene
{
    private Done d;
    ...
    private void createBroomButton()
    {
        ...
        if (!(Inspection.getBroom()))
            c.createButtons("buttonBroom", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate {lis_broomInspection(); }),
            false, true);
        else if ((Inspection.getBroom()))
            c.createButtons("buttonBroom", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate { lis_broomInspection(); }),
            false, false);
    }
    ...
    public void lis_broomInspection()
    {
        destroyButtons();
