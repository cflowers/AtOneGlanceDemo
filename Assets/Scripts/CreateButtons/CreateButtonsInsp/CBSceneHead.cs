...
public class CBSceneHead : AbstractCBScene
{
    private Done d;
    ...
    private void createHeadButton()
    {
        ...
        if (!(Inspection.getHead()))
        c.createButtons("buttonHead", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate {lis_inspection(); }),
          false, true);
        
        else if (Inspection.getHead())
            c.createButtons("buttonHead", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate { lis_inspection(); }),
         false, false);
    }
