...
public class CBSceneAnthraxBook : AbstractCBScene
{
    ...
    void createInspectButton()
    {
        dicAnchor["anchorMin"] = new Vector2(0.5f, 0.5f);
        dicAnchor["anchorMax"] = new Vector2(0.5f, 0.5f);
        dicAnchor["buttonPos"] = new Vector2(36, 28);
        if (!(Inspection.getAnthraxBook()))
        c.createButtons("buttonBookInspect", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate {lis_inspection(); }),
          false, true);
        else if (Inspection.getAnthraxBook())
        c.createButtons("buttonBookInspect", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate { lis_inspection(); }),
        false, false);
    }
