...
public class CBSceneStatue : AbstractCBScene
{
    private Done d;
    ...
    private void createStatueButton()
    {
        dicAnchor["anchorMin"] = new Vector2(0.5f, 0.5f);
        dicAnchor["anchorMax"] = new Vector2(0.5f, 0.5f);
        dicAnchor["buttonPos"] = new Vector2(36, 28);
        if (!(Inspection.getStatue()))
        c.createButtons("buttonStatue", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate {lis_statue(); }),
          false, true);
        else if (Inspection.getStatue())
        c.createButtons("buttonStatue", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate { lis_statue(); }),
         false, false);
    }
