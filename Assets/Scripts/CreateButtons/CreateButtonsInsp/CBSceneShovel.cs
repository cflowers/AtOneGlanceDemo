...
public class CBSceneShovel : AbstractCBScene
{
    private Done d;
    ...
    private void createShovelButton()
    {
        ...
        if (!(Inspection.getShovel()))
        c.createButtons("buttonShovel", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate {lis_shovelInspection(); }),
          false, true);
        else if (Inspection.getShovel())
         c.createButtons("buttonShovel", c.getCanvas().GetComponent<Canvas>().transform, dicAnchor, new UnityAction(delegate { lis_shovelInspection(); }),
          false, false);
    }
    ...
    public void lis_shovelInspection()
    {
        destroyButtons();
